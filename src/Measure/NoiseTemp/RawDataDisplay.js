...
export default function RawDataDisplay(props) {
  ...
  const dataListRow = (item, index) => {
    ...
    let t1 = '';
    if (rawDataRecords) {
      let pol0_filtered_data = rawDataRecords.filter((obj) => {
        return obj.Pol == 0 && obj[item] !== 0
      })
      if (pol0_filtered_data.length > 0) {
        let last = pol0_filtered_data.length - 1;
        t0 = pol0_filtered_data[last][item];
        if (t0 === 0) {
          t0 = '';
        } else {
    ...
    if (rawDataRecords) {
      let pol1_filtered_data = rawDataRecords.filter((obj) => {
        return obj.Pol == 1 && obj[item] !== 0
      })
      if (pol1_filtered_data.length > 0) {
        let last = pol1_filtered_data.length - 1;
        t0 = pol1_filtered_data[last][item];
        if (t0 === 0) {
          t0 = '';
        } else {
          t0 = Number(t0).toFixed(2);
  ...
  const FreqLO = rawDataRecords && rawDataRecords.length ? rawDataRecords[0].FreqLO.toFixed(2) + ' GHz' : null;
  const CenterIF = rawDataRecords && rawDataRecords.length ? rawDataRecords[0].CenterIF.toFixed(2) + ' GHz' : null;
  const TColdLoad = rawDataRecords && rawDataRecords.length ? rawDataRecords[0].TColdLoad.toFixed(2) + ' K' : null;
  const TRF_Hot = rawDataRecords && rawDataRecords.length ? rawDataRecords[0].TRF_Hot.toFixed(2) + ' K' : null;
  const IF_Attn = rawDataRecords && rawDataRecords.length ? rawDataRecords[0].IF_Attn.toFixed(2) + ' dB' : null;
